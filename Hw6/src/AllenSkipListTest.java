...
public class AllenSkipListTest {
    ...
    /**
     * Make sure data in a node actually gets replaced. Also see
     * https://piazza.com/class/i4ppb3mq17h2rh?cid=521.
     */
    @Test()
    public void replaceMultiLevel() {
        ...
        Node<TinyMapEntry> bottom = skipTM.getHead();
        while (bottom.getDown() != null) {
            bottom = bottom.getDown();
        }

        Node<TinyMapEntry> onlyElem = bottom.getNext();
        assertNotNull(onlyElem);
        assertEquals(7, (int) onlyElem.getData().key);
        assertEquals("hey", onlyElem.getData().value);
        assertNull(onlyElem.getNext());
        assertNull(onlyElem.getDown());
